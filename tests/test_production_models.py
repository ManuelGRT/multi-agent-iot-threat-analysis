...
def test_detector_preserves_splits_fits_vectorizer_on_train_and_reports_metrics():
    factory = _CapturingFactory()
    detector = train_global_detector(_candidate_records(), estimator_factory=factory)

    assert [call["task"] for call in factory.calls] == ["binary_detection"]
...
def test_small_real_xgboost_smoke_trains_and_serializes_detector(tmp_path):
    ...
    assert result.report["parameters"]["n_estimators"] == 2
    assert result.model.predict_proba([{"a_signal": 0.0}]).shape == (1, 2)
